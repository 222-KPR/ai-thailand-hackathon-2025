...
def process_pest_detection(self, image_data: str, confidence_threshold: float = 0.01, return_details: bool = False):
    ...
    try:
        ...
        with httpx.Client(timeout=120.0) as client:
            response = client.post(
                f"{VISION_SERVICE_URL}/detect/pests",
                files=files,
                data=data
            )
...
@app.task(bind=True, autoretry_for=(Exception,), retry_kwargs={'max_retries': 3, 'countdown': 60})
def process_disease_detection(self, image_data: str, custom_prompt: Optional[str] = None):
    ...
    try:
        ...
        with httpx.Client(timeout=180.0) as client:  # Longer timeout for LLaVA
            response = client.post(
                f"{VISION_SERVICE_URL}/detect/disease",
                files=files,
                data=data
            )
        ...
        if response.status_code == 200:
            ...
            result['metadata'] = metadata
            result['task_id'] = task_id
            disease_name = result.get('data', {}).get('disease_analysis', {}).get('disease_name', 'Unknown')
            logger.info("Disease detection completed", task_id=task_id, status="success", disease=disease_name)
            return result
        else:
            error_msg = f"Vision service error: {response.status_code} - {response.text}"
...
def process_comprehensive_analysis(self, image_data: str, pest_confidence: float = 0.01, 
    ...
    try:
        ...
        with httpx.Client(timeout=300.0) as client:  # Extended timeout for both models
            response = client.post(
                f"{VISION_SERVICE_URL}/analyze/comprehensive",
                files=files,
                data=data
            )
